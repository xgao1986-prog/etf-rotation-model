...
## 2026-06-21（本次 - Phase 8.1: 排名位置预测力诊断）
...
- 排名范围仅包含16只行业ETF，排除黄金和国债
- 使用B0.3原始评分与排序逻辑
- 不修改策略、评分、交易规则或仓位
- 研究区间：2019-08-13 至 2024-12-31（训练期2019-2022，验证期2023-2024）
- 禁止使用已封存的2025-2026 OOS

**样本构造：**
- 在每个周四调仓截面，使用T日收盘后可获得的信号
- 对16只行业ETF按total_score排序，保存Rank 1-16
- 同时记录BUY条件（trend>=15, confirm>=4, total>=40, prev_close>ma20, ma20_slope>0）
- 信号在T日收盘后生成，从T+1开盘计算未来收益
- 分别计算5/10/20个交易日未来收益

**统计方法：**
- 按排名位置聚合（不是按ETF代码聚合）
- Block bootstrap置信区间：按调仓日期block重采样，1000次，95% CI
- 由于5/10/20日样本重叠，不能将每只ETF样本视为独立

**核心结果：**

| 指标 | 5D | 10D | 20D |
|------|----|-----|-----|
| Rank 1均值 | 0.39% | 0.63% | 0.62% |
| Rank 5均值 | 0.21% | 0.42% | 1.05% |
| Rank 1 vs Rank 5差 | +0.18% | +0.21% | -0.44% |
| 训练期Top5-Bottom5 | +0.39% | +1.16% | +1.10% |
| 验证期Top5-Bottom5 | +0.07% | +0.01% | -0.11% |
| 方向一致性 | 一致 | 一致 | **不一致** |

**Block Bootstrap 95% CI：**
- 5D：仅Rank 3 CI不包含0（0.17%~1.20%），其余Rank CI均包含0
- 10D：Rank 3和Rank 8 CI不包含0，其余包含0
- 20D：仅Rank 8 CI不包含0（0.20%~1.57%），其余包含0
- **结论**：大多数排名的平均收益在统计上不显著
...
**区分度检验：**
- Rank 1-5内部无稳定递减：20D时Rank 3(1.52%) > Rank 1(0.62%)
- 正收益胜率不随排名单调下降
- 超额胜率不随排名单调下降
- 训练期与验证期方向在20D不一致
- 不是单一年份贡献，但验证期效果弱于训练期

**结论分支：情况B**
> Top 5优于其他排名，但Rank 1-5内部无稳定区分度。说明评分适合筛选Top 5，但不适合决定内部权重。

**建议**：继续Top 5等权，不测试Rank 1重仓。不进入Phase 8.2。

**文件：**
- 新增 `scripts/phase8_1_rank_position_diagnostics.py` - 诊断脚本
- 新增 `reports/phase8_1_rank_position_diagnostics.md` - 诊断报告
- 新增 `reports/phase8_1_rank_samples.csv` - 每期排名样本
- 新增 `reports/phase8_1_rank_summary.csv` - 排名汇总统计
- 新增 `reports/phase8_1_rank_diagnostics.png` - 可视化图表
...
**Commit：** 4a4c158（Phase 7.1 v4 修订） → 本次提交
