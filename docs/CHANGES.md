...
## 2026-06-23（本次 - v1.3 Step 4: 80/20组合结构机制拆解）

**目标：** 在不修改引擎核心逻辑的前提下，仅通过修改配置参数运行4个组合结构实验，对比不同行业槽位/防御槽位/单仓上限对收益、风险、交易成本、持仓结构的影响。

**实验设计：**
1. **B0.4（基线）**：stock_max=5, total_max=5, defense_max=2, per_etf=20%。
2. **方案A（4行业+无防御）**：stock_max=4, total_max=4, defense_max=0, per_etf=20%。
3. **方案B（4行业+1防御）**：stock_max=4, total_max=5, defense_max=1, per_etf=20%。
4. **方案C（5行业×16%）**：stock_max=5, total_max=5, defense_max=0, per_etf=16%。

**核心结果：**
- B0.4 精确复现：NAV=2,761,288.07，交易804笔 ✓。
- 方案A：总收益152.97%，CAGR 15.14%，夏普0.89，最大回撤-16.66%，现金占比49.1%。无防御导致大量现金闲置。
- 方案B：总收益180.91%，CAGR 16.99%，夏普0.98，最大回撤-16.38%，为4方案中夏普最高、回撤最小、总收益最高。
- 方案C：总收益118.34%，CAGR 12.59%，夏普0.81，最大回撤-16.27%，现金占比53.18%，单仓上限过低导致资金利用不足。
...
- 黄金/国债贡献：B0.4 黄金+4.96%、国债+2.39%；B 黄金+8.30%、国债+2.53%。防御资产在方案B中贡献更显著。
- 换手率：B0.4 19.44x，A 13.57x，B 15.89x，C 12.59x。

**关键发现：**
- **方案B为最优结构**：在4行业槽位基础上保留1个防御槽位，既提高了夏普（0.98）又降低了最大回撤（-16.38%），总收益也最高（180.91%）。
- **无防御（方案A）导致现金过多**：现金占比49.1%，拖累了收益和夏普。
- **单仓16%（方案C）导致资金利用不足**：现金占比53.18%，即使行业槽位满也难以打满仓位。
- **防御资产对回撤平滑有显著贡献**：黄金和国债在B0.4和方案B中均提供了正收益贡献和回撤缓冲。
...
- metrics.csv：包含4方案的全样本及分时期指标。
- daily_attribution.csv：逐日黄金/国债/行业持仓归因。
- 滑点压力测试：0/3/5/10bp NAV单调递减，无伪改善。
- 数据勾稽：B0.4 NAV=2,761,288.07，交易804笔，与冻结基线一致。

**修改文件：**
- `scripts/v1_3_step4_portfolio_structure_ab.py` — 新建实验脚本
- `reports/v1_3_step4_portfolio_structure_ab.md` — 实验报告
- `reports/v1_3_step4_portfolio_metrics.csv` — 指标汇总
- `reports/v1_3_step4_portfolio_daily_attribution.csv` — 逐日归因
- `docs/CURRENT_STATE.md` — 添加Step 4结论、更新日期/下一步
