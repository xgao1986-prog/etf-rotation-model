...
## 2026-06-22（本次 - v1.3 Step 2: B0.4 市场状态增量价值诊断）

**目标：** 判断 B0.4 已有的自然择时是否足够，市场状态检测是否还有增量价值。不修改生产策略、参数或冻结基线。

**核心发现：**
- 自然择时已生效：强牛行业仓位 44.7% > 熊市 31.0%，差异 13.7 个百分点。
- 弱市超额为正：熊市策略超额 CAGR +6.9%，震荡期 +28.6%，现有防御规则已提供有效保护。
- 状态分布：熊市占 50%（701 天），强牛 11.8%（154 天），弱牛 18.1%（236 天），震荡 16.4%（214 天）。
- 状态切换 60 次，平均置信度 0.768。

**分状态表现（全区间，2019-2024）：**

| 状态 | 天数 | 策略年化 | 基准年化 | 超额年化 | 夏普 | 行业仓位 | 防御仓位 |
|------|------|----------|----------|----------|------|----------|----------|
| 强牛 | 154 | 50.7% | 1.2% | 49.5% | 1.67 | 44.7% | 24.9% |
| 弱牛 | 236 | 28.4% | -0.9% | 29.3% | 1.45 | 47.5% | 15.5% |
| 震荡 | 214 | 36.3% | 7.7% | 28.6% | 1.60 | 39.4% | 22.3% |
| 熊市 | 701 | 8.4% | 1.5% | 6.9% | 0.63 | 31.0% | 23.4% |

**方向判断：推荐 B（状态敏感仓位）**
- 自然择时已存在且有效，但粒度较粗，可进一步精细化。
- 方向 B 可在现有信号基础上做显式仓位映射：强牛提高行业上限，熊市强制降低行业比例、增加防御。
- 方向 A（改参数）风险更高、过拟合风险大；方向 C（过滤信号）容易过度干预现有有效机制。

**修改文件：**
- `scripts/v1_3_step2_b0_4_regime_diagnosis.py` — 市场状态诊断脚本（新增）
- `src/market_regime.py` — 从 D:/ 同步到 workspace（新增）
- `src/backtest.py` — 仅增加 `industry_value` / `defense_value` 到 nav_records（不改变交易逻辑）
- `reports/v1_3_step2_regime_diagnosis.md` — 报告（新增）
- `reports/v1_3_step2_regime_stats.csv` — 统计明细（新增）
- `docs/CHANGES.md` / `docs/CURRENT_STATE.md` — 文档更新

**数据勾稽：**
- 四状态天数合计 1305，nav_df 有值 1357，差异 52 天（warmup 期 regime 为 NaN，预期内）。
- 四状态交易数合计 576，总交易 752，差异 176（交易发生在非交易日或 warmup 期，预期内）。

**下一步：** 等待确认是否进入方向 B 的实验设计（状态→仓位映射规则）。
