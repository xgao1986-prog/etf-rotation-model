...
## 2026-06-23（本次 - v1.3 Step 3: 信号失效退出有效性归因）

**目标：** 诊断 B0.4 中 341 笔 BUY_CONDITION_FAILED 退出的有效性，判断"被误杀"（卖飞）vs"有效避损"vs"震荡往返"比例，为下一步 holding stability 实验提供依据。不修改策略、参数或冻结基线。

**新增内容：**
1. **脚本**：`scripts/v1_3_step3_exit_effectiveness.py`
   - 复用 Step 1 分类逻辑提取 341 笔 BUY_CONDITION_FAILED（reason='调出候选列表' 且 signal_type != 'BUY'）。
   - 计算退出后 5/10/20 日收益（open-to-open，有效交易日，非自然日）。
   - 计算最大上涨/最大下跌。
   - 追踪重新买回：记录后续首次 BUY 的日期、价格、佣金，计算间隔天数和价差收益。
   - 按预设规则分类：震荡往返（优先级1）→ 误杀卖飞（20日收益≥+3%或最大上涨≥+5%）→ 有效避损（20日收益≤-3%或最大下跌≤-5%）→ 中性。
2. **报告**：`reports/v1_3_step3_exit_effectiveness.md`
   - 总体分类结果（研究期/验证期/样本外三层）。
   - 重新买回统计（平均间隔、价差收益、往返佣金）。
   - 分层分析指向 `v1_3_step3_exit_summary.csv`。
   - 决策建议：研究期震荡往返 42.4%、验证期 26.0%，均>15%，建议设计 holding stability 实验。
3. **CSV 数据**：`reports/v1_3_step3_exit_events.csv`（339笔事件明细）和 `v1_3_step3_exit_summary.csv`（分层汇总）。
4. **文档**：`docs/CURRENT_STATE.md`（添加 Step 3 结论，更新下一步任务）、`docs/CHANGES.md`（添加本条目）。

**关键发现：**
- 341 笔提取，339 笔可分析（2 笔因卖出日期后无未来交易日数据，标记为数据不足）。
- 分类结果（研究期 → 验证期）：有效避损 34.1% → 33.3%，误杀卖飞 11.2% → 24.0%，震荡往返 42.4% → 26.0%，中性 12.4% → 16.7%。
- 重新买回：研究期 100% 重新买回（平均间隔 39.4 个交易日，价差收益 0.37%）；验证期 99% 重新买回（平均间隔 64.9 个交易日，价差收益 0.15%）。
- 方向一致性：震荡往返和误杀卖飞两期均存在，验证期误杀卖飞比例上升。
- 决策建议：两期震荡往返比例均>15%，建议下一步设计 holding stability 实验。

**不修改：**
- B0.4 策略、参数、交易逻辑或冻结基线。
- Step 1 / Step 2 的脚本、报告或 CSV。

**验证：**
- 脚本运行成功，生成 3 个文件。
- 样本数：339 笔分析（2 笔跳过已标记）。
- 样本外仅列出，不参与结论。
- 禁止未来函数：所有计算基于卖出日期之前已知的信号和价格。

**修改文件：**
- `scripts/v1_3_step3_exit_effectiveness.py` — 新增
- `reports/v1_3_step3_exit_effectiveness.md` — 新增
- `reports/v1_3_step3_exit_events.csv` — 新增
- `reports/v1_3_step3_exit_summary.csv` — 新增
- `docs/CURRENT_STATE.md` — 添加 Step 3 结论、更新日期/提交/下一步
