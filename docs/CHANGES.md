...
## 2026-06-22（本次 - v1.3 Step 2: B0.4 市场状态增量价值诊断，修正版）

**目标：** 判断 B0.4 已有的自然择时是否足够，市场状态检测是否还有增量价值。不修改生产策略、参数或冻结基线。

**修正内容（相对于初版）：**
1. **修正仓位数字矛盾**：初版 CHANGES 中强牛行业仓位 44.7%/熊市 31.0% 为错误数据（来自 workspace 版本的不一致运行）。以脚本输出 CSV 为唯一数值来源，修正为：强牛行业仓位 74.9%，熊市 42.0%。
2. **修正候选方向定义**：初版错误地将"状态敏感仓位"称为方向 B。正确定义为：A=仅调整总仓位，B=仅调整买入门槛，C=仅调整防御比例。
3. **修正方向推荐**：初版推荐"B（状态敏感仓位）"，修正为"证据不足，暂不推荐"。理由：自然择时已覆盖三个方向的大部分价值（总持仓强牛85.4%→熊市59.1%，防御比例10.5%→17.1%），弱市已有正超额，增量价值不确定。
4. **增加小样本警告**：研究期强牛仅60天，明确标注样本不足；验证期震荡45天、弱牛71天、强牛94天同样存在年化膨胀问题。
5. **增加详细勾稽章节**：状态天数勾稽（1305 vs 1307，差异2=warmup NaN）、交易数勾稽（642 vs 804，差异162=regime为NaN的交易日）、收益连乘勾稽（状态收益不能简单连乘）。
6. **增加未归因交易调查**：162笔未归因交易中，22笔发生在warmup期（regime为NaN），57天交易日期不在nav_df中，真实原因为regime检测覆盖不完整。
...
| 状态 | 天数 | 期间收益 | 策略年化 | 基准年化 | 超额年化 | 行业仓位 | 防御仓位 | 现金仓位 |
|------|------|----------|----------|----------|----------|----------|----------|----------|
