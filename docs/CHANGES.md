...
## 2026-06-20（本次 - Phase 6.7 长假调仓日历适配实验 + Phase 6.6 春节Deep Dive修正）

**修改了什么：**
- Phase 6.7 长假调仓日历适配实验：
  - 比较4种调仓规则：A(当前周四)、B(前补)、C(后补)、D(最近交易日)
  - 有效样本：2019-2024共16个长假（7个春节/国庆+9个其他），2025-2026封存
  - 结果：A(当前)全面最优，B与A完全相同，C全面劣化（总收益-33%），D轻微劣化（总收益-0.56%）
  - 结论：保持当前周四调仓规则，不采纳任何日历适配
- Phase 6.6 春节Deep Dive修正（基于用户反馈）：
  - 窗口收益加入前一日NAV，确保10日窗口含完整10段收益，节后含跳空
  - 春节整体收益使用连乘计算，不得直接相加
  - 明确有效样本为2020-2026共7次（2019年策略未开始，排除）
  - 删除"防御资产拖累"假设（各窗口防御占比均为0%，无证据）
  - 2025-2026仅描述，不用于规则生成
  - 结论：春节无稳定负效应，2020年春节策略跑赢基准，2020全年弱势非春节造成

**改了哪些文件：**
- 新增 `scripts/phase6_7_holiday_rebalance_experiment.py` — 长假调仓日历实验脚本
- 新增 `reports/phase6_7_holiday_rebalance_experiment.md` — 实验报告
- 新增 `scripts/phase6_6_spring_festival_deep_dive.py` — 春节Deep Dive诊断脚本（修正版）
- 更新 `reports/phase6_6_spring_festival_deep_dive.md` — 修正后报告
- 更新 `docs/CURRENT_STATE.md` — 添加Phase 6.6和6.7摘要
