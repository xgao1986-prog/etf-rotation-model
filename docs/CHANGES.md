...
## 2026-06-23（本次 - v1.3 Step 3 v2: 信号失效退出有效性归因修正版）

**目标：** 修正Step 3 v1的7项问题，完整保留341笔事件，禁止残缺窗口判断，统一15%阈值，不修改策略、参数或冻结基线。

**修复内容（7项）：**
1. **完整保留341笔事件**：不再因无未来行情而continue跳过。CSV严格341行。新增observation_status（COMPLETE_20D/CENSORED_20D/NO_FUTURE）和available_future_days列。样本外4笔截尾不足、2笔无未来数据，均标记为"数据不足"主分类。
2. **禁止残缺窗口判断20日指标**：只有COMPLETE_20D样本才参与20日收益、20日最大涨跌、误杀卖飞、有效避损分类。CENSORED_20D和NO_FUTURE样本主分类强制为"数据不足"，不根据部分窗口判断。研究期/验证期统计以完整20日样本为分母，并披露排除数量。
3. **重新买回统计分开输出**：三层指标分别披露——任意未来首次买回率（研究期100%/验证期99.0%）、20个交易日内买回率（43.5%/29.2%）、20日内且买回价≥卖出价的震荡往返率（42.4%/26.0%）。不再用最终接近100%的买回率替代20日指标。
4. **往返佣金修正**：从仅rebuy_commission修正为sell_commission + rebuy_commission。分别输出全部买回样本和震荡往返样本的总额、均值。研究期：全部买回均值147.53元/笔，震荡往返均值146.31元/笔；验证期：全部买回均值176.71元/笔，震荡往返均值175.14元/笔。
5. **补齐分层分析**：新增持有时间区间（303笔<30d）、退出时total_score区间（339笔≥0.8）、信号失效条件。信号失效条件因scores_df中不存在ma_condition/multi_condition列，全部341笔标记为condition_unknown，已明确说明原因。
6. **修正方向一致性阈值**：统一使用15%阈值，不再混用10%。震荡往返研究期42.4%/验证期26.0%两期均>15%（是）；误杀卖飞研究期11.2%/验证期24.0%不满足两期均>15%（否）。
7. **增加独立数据勾稽章节**：341 = 完整20日(335) + 截尾不足(4) + 无未来数据(2)。研究期170=完整20日(170)，验证期96=完整20日(96)，样本外75=完整20日(69)+截尾不足(4)+无未来数据(2)。主分类合计与适用分母一致。events.csv严格341行。

**关键发现（v2修正后）：**
- 数据勾稽：341 = 研究期170 + 验证期96 + 样本外75。完整20日335笔，截尾不足4笔，无未来数据2笔。
- 分类结果（仅完整20日样本，研究期→验证期）：有效避损34.1%→33.3%，误杀卖飞11.2%→24.0%，震荡往返42.4%→26.0%，中性12.4%→16.7%。
- 重新买回：任意未来首次买回率100%/99.0%；20日内买回率43.5%/29.2%；20日内震荡往返率42.4%/26.0%。
...
**修改文件：**
- `scripts/v1_3_step3_exit_effectiveness.py` — v2重写（完整保留341笔、禁止残缺窗口、三层买回率、佣金修正、分层分析、15%阈值、数据勾稽）
- `reports/v1_3_step3_exit_effectiveness.md` — v2重新生成
- `reports/v1_3_step3_exit_events.csv` — v2重新生成（严格341行）
- `reports/v1_3_step3_exit_summary.csv` — v2重新生成
- `docs/CURRENT_STATE.md` — 添加Step 3 v2结论、更新日期/提交/下一步
