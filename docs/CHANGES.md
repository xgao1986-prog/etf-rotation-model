...
## 2026-06-20（本次 - Phase 6.7 长假调仓日历适配实验修正版）

**修改了什么：**
- Phase 6.7 长假调仓日历适配实验（修正版）：
  - 使用自然日生成计划周四，不在trading_days中搜索休市周四
  - B方案：长假覆盖计划周四 → 节前最后交易日替代；删除被替代周四；输出13次替代清单
  - C方案：节后首个交易日替代假期后正常周四；删除被替代周四；7次替代
  - D方案：每周最多一次，替代而非新增；7次替代
  - 增加断言：每次替代有source_date和target_date；同一长假仅替代一次；无<3交易日重复调仓
  - 输出最大自然日/交易日调仓间隔
  - 重跑2019-2024，不运行封存样本
  - 结果修正：A(当前)62.21%最优，B(前补)39.07%劣化37%，C(后补)30.15%劣化52%，D(最近)61.51%轻微劣化
  - 结论：保持当前周四规则，不采纳任何日历适配

**改了哪些文件：**
- 重写 `scripts/phase6_7_holiday_rebalance_experiment.py` — 修正版脚本
- 更新 `reports/phase6_7_holiday_rebalance_experiment.md` — 修正版报告（含结论、间隔统计、替代记录）
