...
**当前分支**：`feature/paper-trading-v0.1`
**当前版本**：v1.2.3
**当前 HEAD**：`365a7f4` 及之后文档收口提交
...
- **Paper Trading v0.1.0：Phase 1 终审通过**
  - 独立虚拟账户数据库（SQLite），不修改 B0.4 数据库和策略代码
  - 支持现金启动和导入持仓启动，NAV 勾稽验证
  - 不可变策略配置快照、同日重复事件阻止（dedupe_key）
  - 账户管理命令：`create-cash` / `list` / `inspect`
  - 15 个 Phase 1 测试全部通过，30 个现有测试无回归
- **实盘助手 v0.2：已交付骨架**
...
- `tests/test_b0_4_slippage.py`：8 passed（0bp 复现 NAV=2,761,288.07，3bp 无静默跳过，NAV 单调递减）
...
**Phase 2：单个 B0.4 虚拟账户自动运行**

- 让一个 B0.4 对比盘完成每日估值、周度信号和模拟成交
- 自动行情更新、周四信号、周五 T+1 开盘成交
- 不进入 Phase 2 功能开发前，不做任何自动运行代码

**Phase 2 开始前的前置任务（独立）**：
- B0.4 冻结快照测试口径修复：`test_0bp_matches_baseline` 应读取冻结快照而非当前数据库；运行前校验冻结数据哈希；当前数据库只用于实盘数据，不作为冻结基线测试输入
...
**虚拟实盘状态**：Phase 1 独立账户账本已终审通过（v0.1.0）。

**版本口径**：
- 策略版本：v1.2.3（B0.4 基线）
- 调仓引擎：v2.5
- 虚拟实盘：v0.1.0（Phase 1 账本）
