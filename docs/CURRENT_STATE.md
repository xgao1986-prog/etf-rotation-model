...
- B. 假期前补调：长假覆盖计划周四 → 节前最后交易日替代（346个调仓日，13次替代）
- C. 假期后补调：节后首个交易日替代假期后正常周四（341个调仓日，7次替代）
- D. 最近交易日：计划周四休市时选最近交易日，每周最多一次（354个调仓日，7次替代）

**修正要点：**
1. 计划周四基于自然日生成，不在trading_days中搜索
2. B/C/D均记录source_date和target_date，删除被替代周四
3. 断言检查：所有替代有记录、同一长假仅替代一次、无<3交易日重复调仓
4. 输出最大自然日和交易日调仓间隔
...
**分析：**
- **A vs B**：B劣化-37%（62.21%→39.07%）。前补增加了8次非周四节前调仓（如2019-09-30、2020-09-30），这些额外调仓恰在市场转折点附近，导致额外亏损。同时部分正常周四因间隔<3被跳过。
- **A vs C**：C全面劣化-52%（62.21%→30.15%）。后补强制在节后首日调仓，信号不稳定，收益腰斩。
- **A vs D**：D轻微劣化-1.1%（62.21%→61.51%）。额外7次调仓增加交易成本，无风险调整后收益改善。

**结论：**
> **保持当前周四调仓规则（A），不采纳任何长假日历适配规则。**

理由：
1. B（前补）劣化37%：节前调仓增加噪声，且跳过部分正常周四
2. C（后补）劣化52%：节后首日信号不稳定，收益腰斩
