...
- **B0.4 单变量滑点敏感性测试已完成**：
  ...
  - 3bp 滑点使 NAV 下降约 6.4%（2,761,288 → 2,584,887），年化从 16.68% → 14.47%。
  - 5bp 滑点使 NAV 下降约 11.3%（2,761,288 → 2,450,247），年化从 16.68% → 12.78%。
  - 10bp 滑点使 NAV 下降约 17.9%（2,761,288 → 2,267,246），年化从 16.68% → 11.17%。
  - 交易次数随滑点减少，归因于可用现金不足导致部分买入被跳过，改变持仓路径。
  - 6 项自动测试全部通过（0bp 一致性、价格方向、NAV 单调递减、滑点成本恒等式）。
- **下一阶段**：B0.4 的滑点测试已提供基准数据。下一步可基于这些结果进行策略迭代或新的单变量实验。
...
- `tests/test_app_b0_signature.py`：6 passed。
  ...
  - 开启动量或波动率因子后签名偏离。
  - 修改止损、权重或最大持仓数后签名偏离。
- `tests/test_b0_4_slippage.py`：6 passed。
  - 0bp 完美复现 B0.4（NAV=2,761,288.07，交易804笔）。
  - 3bp 买入价格全部高于 0bp（成交价上调）。
  - 3bp 卖出价格全部低于 0bp（成交价下调）。
  - 0bp>3bp>5bp>10bp NAV 单调递减。
  - 滑点成本占 NAV 差异 25%~60%（路径变化影响其余部分）。
  - 0bp 总佣金 ≈ 68,826（与 B0.4 一致）。
...
- 0bp/3bp/5bp/10bp 四组结果已生成。
- 6 项自动测试全部通过。
- 报告已保存至 `reports/b0_4_slippage_sensitivity.md`。

### 待决定：下一个单变量实验方向

基于当前结果，可考虑的方向：
1. **执行价格口径测试**：当前使用 `open`，测试 `close` 或 `vwap` 口径的影响。
2. **整手限制敏感性**：测试 lot_size=100 改为 1（无整手限制）对结果的影响。
3. **防御资产配比**：测试不同 `DEFENSE_ALLOCATION` 配置对回撤和收益的影响。
4. **样本外验证**：使用 2024 年后数据作为样本外，验证 B0.4 的泛化能力。
