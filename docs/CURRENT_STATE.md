...
**最后更新**：2026-06-20（Phase 6.7 最终修正完成：长假调仓日历适配实验，截断交易日、修正C方案、断言终止、B0.3严格复现、日历纯函数测试，结论保持当前周四规则）
...
4. 间隔验证只检查新增替代与相邻调仓日的间隔，不检查正常周四
5. 严格复现B0.3：使用build_config()，fallback_equity_enabled=False，加载完整ETF
6. B方案区分被替代计划周四（13个）和实际新增调仓日（12个）
7. 日历纯函数测试6个：日期截断、C无同日替代、目标都是交易日、被替代已删除、验证失败中止、输入顺序无关

**比较4种规则：**
- A. 当前规则：仅周四，休市则跳过（275个调仓日）
- B. 假期前补调：长假覆盖计划周四 → 节前最后交易日替代（274个调仓日，13次替代）
- C. 假期后补调：节后首个交易日替代假期后正常周四（272个调仓日，3次替代）
- D. 最近交易日：计划周四休市时选最近交易日，每周最多一次（272个调仓日，5次替代）
...
**分析：**
- **A vs B**：B劣化23.00%。前补13次替代中，8次替代了非周四的节前交易日（如2019-09-30周一、2020-09-30周三），同时跳过了3个正常周四（因间隔<3）。净效果：减少1个调仓日，但增加噪声。
- **A vs C**：C全面劣化34.28%。后补3次替代（2019-10-08、2020-05-06、2024-10-08），同时因间隔<3跳过3个正常周四。净效果：减少3个调仓日，收益大幅下降。
- **A vs D**：D劣化6.59%。5次替代（前补或后补），同时因间隔<3跳过8个正常周四。净效果：减少3个调仓日，无风险调整后收益改善。

**结论：**
> **保持当前周四调仓规则（A），不采纳任何长假日历适配规则。**

理由：
1. B（前补）劣化23%：节前调仓增加噪声，且跳过部分正常周四
2. C（后补）劣化34%：节后首日信号不稳定，收益大幅下降
3. D（最近）劣化7%：额外调仓增加成本，无改善
