...
**最后更新**：2026-06-23（Step 3 v2: 信号失效退出有效性归因修正版）  
**工作目录**：`D:\etf_rotation_model`  
**当前分支**：`feature/v1.3-regime-research`  
**当前版本**：v1.2.3
**当前研究提交**：`0e8d73d`（v1.3 Step 3 v2: 信号失效退出有效性归因修正版）  
...
- **v1.3 Step 2: B0.4 市场状态增量价值诊断 已完成（修正版v3）**：
  ...
  - 不修改 B0.4 策略、参数或冻结基线。
- **v1.3 Step 3 v2: 信号失效退出有效性归因 已完成（修正版）**：
  - 完整保留 341 笔 BUY_CONDITION_FAILED 退出，不因无未来行情而跳过。CSV 严格 341 行。
  - 增加 observation_status（COMPLETE_20D/CENSORED_20D/NO_FUTURE）和 available_future_days 列。
  - 禁止用不足 20 日的残缺窗口判断 20 日收益、最大涨跌、误杀卖飞、有效避损。截尾/无未来数据样本主分类强制为"数据不足"。
  - 数据勾稽：研究期 170 = 完整20日(170) + 截尾不足(0) + 无未来数据(0)；验证期 96 = 完整20日(96) + 截尾不足(0) + 无未来数据(0)；样本外 75 = 完整20日(69) + 截尾不足(4) + 无未来数据(2)。总计 341 笔。
  - 分类结果（仅完整20日样本，研究期 → 验证期）：有效避损 34.1% → 33.3%，误杀卖飞 11.2% → 24.0%，震荡往返 42.4% → 26.0%，中性 12.4% → 16.7%。
  - 重新买回统计（分三层）：任意未来首次买回率 研究期100% / 验证期99.0%；20个交易日内买回率 43.5% / 29.2%；20日内且买回价≥卖出价的震荡往返率 42.4% / 26.0%。
  - 往返佣金：sell_commission + rebuy_commission。全部买回均值 147.53元/笔（研究期）、176.71元/笔（验证期）；震荡往返均值 146.31元/笔（研究期）、175.14元/笔（验证期）。
  - 分层分析：持有时间区间（303笔<30d）、退出时total_score区间（339笔≥0.8）、信号失效条件（全部341笔标记为condition_unknown，因scores_df中不存在ma_condition/multi_condition列）。
  - 方向一致性（统一15%阈值）：震荡往返 42.4% / 26.0% 两期均>15%（是），误杀卖飞 11.2% / 24.0% 不满足两期均>15%（否）。
  - 决策建议：震荡往返比例两期均>15%，是支持 holding stability 实验的主要证据；误杀卖飞不满足两期均>15%，不能作为独立证据。
  - 交付物：scripts/v1_3_step3_exit_effectiveness.py（v2）、reports/v1_3_step3_exit_effectiveness.md（v2）、reports/v1_3_step3_exit_events.csv（v2）、reports/v1_3_step3_exit_summary.csv（v2）。
  - 不修改 B0.4 策略、参数或冻结基线。
