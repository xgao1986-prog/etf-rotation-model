...
**以 B0.3 为冻结基准，比较 4 个 volatility 使用方案：**

| 方案 | 训练期年化 | 验证期年化 | Rank IC(H10) | vol-momentum 相关 | 结论 |
|------|-----------|-----------|--------------|-------------------|------|
| B0.3 基准 | 9.55% | 13.45% | -0.0122 | -0.0727 | **保持** |
| 高波动前20%加分 | 9.55% | 13.45% | +0.0045 | +0.1864 | 无改善 |
| 高波动+趋势条件 | 9.55% | 13.45% | -0.0137 | **+0.4644** | 无改善，重新引入动量 |
| 波动率加速上升 | 9.55% | 13.68% | -0.0079 | +0.0609 | 训练期无改善 |

**核心发现：**
- 所有 4 个方案在训练期表现**完全一致**（9.55%），说明 vol_score 在当前参数下对 total_score 影响极小
- 方案 C（高波动+趋势条件）vol_score-momentum_rank 相关性高达 **+0.4644**，**重新引入动量效应**
- 方案 D（波动率加速上升）验证期略好（+0.23%），但训练期无改善，不入选
- **结论：保持 B0.3 不变**

**文件**：`scripts/phase5_high_volatility_experiment.py`、`reports/phase5_high_volatility_experiment.md`
