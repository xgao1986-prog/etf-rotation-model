...
### Phase 5.8: 高波动增量价值实验（修正版 v2）

**以 B0.3 为冻结基准，比较 4 个 volatility 使用方案（修正后实验组显式开启 vol）：**

| 方案 | 训练期年化 | 验证期年化 | Rank IC(H10) | vol-momentum 相关 | 结论 |
|------|-----------|-----------|--------------|-------------------|------|
| B0.3 基准 | 9.55% | **13.45%** | -0.0122 | -0.0727 | **保持** |
| 高波动前20%加分 | 9.93% | 9.35% | +0.0045 | +0.1864 | 劣于基准 |
| 高波动+趋势条件 | 9.93% | 9.35% | -0.0137 | **+0.4644** | 劣于基准，重新引入动量 |
| 波动率加速上升(shift1) | **11.11%** | 8.60% | -0.0112 | +0.0612 | 训练期最优但验证期过拟合 |

**核心发现（v2修正后）：**
- **断言通过**：实验组 B/C/D 均有非零 vol_score（46%~78%），total_score 与关闭 vol 时存在差异
- **交易序列不同**：验证期交易次数 B0=232，B/C=255，D=270，说明 vol_score 确实影响了实际交易决策
- **验证期全面劣化**：所有实验组验证期年化（8.60%~9.35%）均显著低于 B0.3 基准（13.45%）
- **方案 C 重新引入动量**：vol_score-momentum_rank 相关性高达 **+0.4644**，与趋势分高度耦合（+0.5787）
- **方案 D 过拟合**：训练期最优（11.11%），但验证期仅 8.60%（-2.51%），典型的训练集过拟合
- **结论：保持 B0.3 不变**。高波动加分策略无增量价值，vol_score 保持关闭。

**修正说明（v2 vs v1）：**
- v1 错误：实验组未显式覆盖 `volatility_factor_enabled=True`，导致 vol_score 被 B0.3 默认配置覆盖为 0
- v1 错误：波动率加速均值未 shift(1)，使用当日数据（未来信息）
- v2 修正：实验组 `compute_total_score` 直接求和，不再调用父类（避免 vol 开关覆盖）
- v2 修正：vol_ma20 使用 `rolling(20).mean().shift(1)`
- v2 新增：断言检查非零 vol_score 和 total_score 差异

**文件**：`scripts/phase5_high_volatility_experiment.py`（v2）、`reports/phase5_high_volatility_experiment.md`（v2）
