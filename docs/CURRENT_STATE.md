...
- **Paper Trading v0.3：Task 6 Streamlit 虚拟盘页面修复中（待 Task 7 验收）**
  ...
  - 已修复：运行日期与行情数据日期不一致时阻止运行；最大回撤正确计算首日下跌；B0.4 策略对比仅在同一批次（同 group_id + start_date）内查找 strategy_name 包含 B0.4 的参照账户
  - 21 个 UI 测试 + 11 项指标测试通过；paper_trading 全量待验收后更新
  - 不修改 B0.4 策略、参数或冻结数据
- **实盘助手 v0.2：已交付骨架**
...
- `tests/test_paper_trading_ui.py`：21 passed（页面渲染、资金不变传递、无策略参数编辑器、运行按钮触发、日期/价格/评分异常阻止运行、价格空值/零/负值阻止运行、评分空值阻止运行、影子订单需确认、确认/拒绝/取消调用服务、拒绝需原因、总览展示绩效指标、详情展示订单与曲线、同一批次 B0.4 对比、排除无批次影子账户）
...
**虚拟实盘状态**：Phase 2 每日运行流程、止损检查、每周调仓、模拟成交已完成；Task 6 Streamlit 虚拟盘页面已集成，关键问题已修复，待 Task 7 浏览器验收后标记完成。

**版本口径**：
- 策略版本：v1.2.3（B0.4 基线）
- 调仓引擎：v2.5
- 虚拟实盘：v0.3-dev（Phase 2 运行流程 + Streamlit 虚拟盘页面 + 影子订单确认流程，待验收）
