# -*- coding: utf-8 -*-
"""
market_regime_attribution_v5b.py - v5b 互斥五类贡献+配置冻结+统一截断2026-06-05

核心口径：
- 逐日逐ETF真实持仓归因，会计式勾稽
- 缺失价格采用最近有效收盘价（与回测引擎一致）
- 构建等权影子组合：同仓位规模下，行业持仓均分给所有可交易ETF
- 超额拆分为互斥五类：仓位暴露、行业选择、交易执行、防御资产、交易成本
- 统一截断到全标的共同截止日 2026-06-05
- 输出完整冻结配置及配置哈希

验收条件：
1. 每日贡献与NAV收益精确勾稽（零误差）
2. 四个状态金额贡献合计与全组合净利润精确勾稽（零误差）
3. 五类贡献互斥可加：暴露+选择+执行+防御+成本=总贡献
...
def format_report_v5(state_stats, daily_df, total_nav_change, max_disc, max_disc_date, industry_tickers, defense_tickers, result):
    ...
    lines.append("# 市场状态归因分析报告（v5b - 互斥五类贡献+配置冻结）")
    lines.append(f"\n**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"**策略**: ETF轮动 v1.2 基线（{len(industry_tickers)}只行业ETF）")
    lines.append(f"**数据口径**: 前复权，统一数据至2026-06-05")
    lines.append(f"**核心口径**: 逐日逐ETF真实持仓归因，会计式勾稽，互斥五类贡献")
    lines.append(f"**修复**: 缺失价格采用最近有效收盘价，禁止持仓市值归零；缺价影响修正为累计58.04万元")
    ...
    cfg_dict = {
        'STRATEGY_CONFIG': {k: v for k, v in config.STRATEGY_CONFIG.items() if not callable(v)},
        'BACKTEST_CONFIG': {k: v for k, v in config.BACKTEST_CONFIG.items() if not callable(v)},
    ...
    cfg_json = json.dumps(cfg_dict, sort_keys=True, ensure_ascii=False, default=str)
    cfg_hash = hashlib.sha256(cfg_json.encode('utf-8')).hexdigest()
    
    lines.append(f"\n**配置哈希 (SHA256)**: `{cfg_hash}`")
    lines.append(f"\n```json")
    ...
    for name in ['强牛', '弱牛', '震荡', '熊市']:
        ...
        lines.append(f"- **行业选择贡献**（既有持仓超额）: {s['selection_contrib']:,.2f}")
        lines.append(f"- 交易执行贡献（当日买卖）: {s['execution_contrib']:,.2f}")
    ...
    if not missing_log.empty:
        total_impact = missing_log['impact'].sum()
        lines.append(f"\n## 缺失价格事件（v5b修正：累计影响约58.04万元）")
        lines.append(f"\n| 日期 | ETF | 连续缺失天数 | 最近有效价格 | 持仓股数 | 当日新增虚假冲击 |")
        lines.append(f"|------|-----|--------------|--------------|----------|------------------|")
        ...
        lines.append(f"\n**累计缺价影响**: {total_impact:,.2f} 元（约 {total_impact/10000:.2f} 万元）")
    else:
        lines.append(f"\n## 缺失价格事件")
        lines.append(f"\n截断至 2026-06-05 后，无缺失价格事件。")
    
    lines.append(f"\n## 版本边界")
    lines.append(f"- v1.2.2 已收口")
    lines.append(f"- v5b 研究阶段：互斥五类贡献口径 + 配置冻结 + 统一截断2026-06-05")
...
def main():
    print("="*80)
    print("v5b 逐日逐ETF真实持仓归因（互斥五类贡献+配置冻结）")
    print("="*80)
    print(f"开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()

    print("[1/5] 加载数据（统一截断至2026-06-05）...")
    ...
    missing_log = result.get('missing_price_log', pd.DataFrame())
    if not missing_log.empty:
        print(f"  缺失价格记录: {len(missing_log)} 条, 累计影响: {missing_log['impact'].sum():,.2f}")
    else:
        print(f"  缺失价格记录: 无")
    ...
    for name in ['强牛', '弱牛', '震荡', '熊市']:
        s = state_stats.get(name, {})
        if s.get('trading_days', 0) > 0:
            sum_check = s['exposure_contrib'] + s['selection_contrib'] + s['execution_contrib'] + s['defense_contrib'] + s['cost_contrib']
            print(f"    {name}: 五类之和={sum_check:,.2f}, 总贡献={s['total_contrib']:,.2f}, 差={sum_check - s['total_contrib']:,.4f}")
    ...
    report_path = 'reports/market_regime_attribution_v5b.md'
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(report)
    print(f"\n报告已保存: {report_path}")

    print(f"\n[OK] v5b 逐日逐ETF真实持仓归因完成")
