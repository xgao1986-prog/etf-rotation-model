...
|--------|---------------|---------|---------|------|------|
| Kimi | `main.py` signal命令 | 2026-06-12 | 2026-06-12 | ⏳ | Codex审计6个Bug已修复，待修复评分信号沉淀，待跑四组可比回测 |

---

## 📋 已完成工作（倒序，最新的在最上面）

### 2026-06-12 10:30 - Kimi - Codex工程审计修复完成（6个Bug）

**Codex审计发现的问题：**
1. ❌ 回测实际用了22个标的，不是16个（main.py读取全库，混入基准+概念ETF）
2. ❌ 大盘择时基本没有生效（backtest.py从bench_df找market_signal，但bench_df无此字段）
3. ❌ 信号生成shift(1)没有按ticker分组，存在跨标的串行污染
4. ❌ 仓位计算基于初始资金，不是实时净值
5. ❌ AKShare ETF后缀判断bug（159xxx标成.SH，应为.SZ）
6. ❌ HANDOFF.md日期混乱（写2025-06-12，实际2026-06-12）

**修复内容：**
- ✅ `main.py`: `db.get_market_data()` → `db.get_market_data(ticker=etf_tickers)`，限定universe为16只
- ✅ `backtest.py`: 大盘择时信号从`signals_df`获取（而非`bench_df`），market_signal真正生效
- ✅ `backtest.py`: 仓位计算从`initial_capital`改为`current_value`（组合实时净值）
- ✅ `strategy.py`: `shift(1)`改为`groupby('ticker')['close'].shift(1)`，消除跨标的串行污染
- ✅ `data_fetcher.py`: 修复AKShare后缀判断，`159xxx`和`16xxx`标为`.SZ`，`5xxx`标为`.SH`
- ✅ `HANDOFF.md`: 统一所有日期为2026-06-12
- ✅ `main.py`: 修复`¥`字符编码错误（GBK不支持）

**修复后回测结果（16只池子，可信）：**
| 指标 | 修复前（混入22只） | 修复后（纯16只） | 变化 |
|------|-------------------|-----------------|------|
| 总收益率 | 37.71% | **31.82%** | -5.89% |
| 年化收益率 | 4.85% | **4.17%** | -0.68% |
| 夏普比率 | 0.41 | **0.40** | -0.01 |
| 最大回撤 | -20.36% | **-17.23%** | **+3.13%（更优）** |
| 交易次数 | 652 | **614** | -38 |
| 交易标的 | 22只（混入基准+概念ETF） | **16只（纯净）** | ✅ |
| 000300.SH交易 | 45次（误交易基准） | **0次** | ✅ |

**关键洞察：**
1. 之前37.71%的收益不可信——混入了22只标的，包括表现较好的概念ETF和误交易的基准
2. 修复后回撤从-20.36%改善到-17.23%——大盘择时真正生效 + 仓位基于净值管理
3. 当前31.82%是**第一个可信的16只池子回测结果**
4. 交易记录验证：16只标的，0次基准误交易，0次概念ETF混入

**待修复（P1）：**
- ⏳ 评分和信号沉淀到数据库（daily_scores=0, trade_signals=0）
- ⏳ 重新跑四组可比回测（旧15/新16/加黄金/加旅游）
- ⏳ 样本内2019-2023 vs 样本外2024-2026分开分析
- ⏳ 无大盘择时/有大盘择时对照实验
...
|---------|--------|------|--------|------|------|---------|------|------|
| 2026-06-12 | Kimi | 2019-06-03 ~ 2026-06-11 | 32.33% | 4.23% | 0.45 | -13.37% | master | **动量排名修复后**：横截面排名生效，回撤大幅改善 |
| 2026-06-12 | Kimi | 2019-06-03 ~ 2023-12-29 | 2.82% | 0.63% | 0.08 | -12.77% | master | 样本内（2019-2023） |
| 2026-06-12 | Kimi | 2024-01-02 ~ 2026-06-11 | 29.59% | 11.71% | 1.04 | -9.39% | master | 样本外（2024-至今） |
| 2026-06-12 | Kimi | 2019-06-03 ~ 2026-06-11 | 31.82% | 4.17% | 0.40 | -17.23% | master | ~~Bug修复后首次回测（动量未修复）~~ |
| 2026-06-12 | Kimi | 2019-06-03 ~ 2026-06-11 | 37.71% | 4.85% | 0.41 | -20.36% | master | ~~混入22只标的，不可信~~ |
