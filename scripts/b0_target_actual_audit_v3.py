# -*- coding: utf-8 -*-
"""B0 Target vs Actual Position Audit v3 - Correct retention logic, 16 ETF pool"""
...
print("=" * 80)
print("B0 Target vs Actual Position Audit v3")
print("=" * 80)
...
# ============================================================
# 3. Per-day target vs actual calculation (with retention logic)
# ============================================================
...
for i, row in nav_df.iterrows():
    ...
    industry_value = 0
    defense_value = 0
    current_holdings = set(positions_detail.keys())
    current_core_holdings = [t for t in current_holdings if t in CORE_TICKERS_SET]
    current_defense_holdings = [t for t in current_holdings if t in DEFENSE_TICKERS_SET]
    ...
    if is_rebalance:
        # Step 1: Sell holdings not in candidates
        # Step 2: Retain holdings that are in candidates
        core_retained = [t for t in current_core_holdings if t in core_candidates]
        defense_retained = [t for t in current_defense_holdings if t in defense_candidates]
        
        n_core_retained = len(core_retained)
        n_defense_retained = len(defense_retained)
        
        # Step 3: Calculate max_new after selling
        total_retained = n_core_retained + n_defense_retained
        max_new = TOTAL_MAX_HOLDINGS - total_retained
        
        # Step 4: Core buy slots = min(new candidates not already held, max_new, stock_max_holdings - retained)
        core_new_candidates = [t for t in core_candidates if t not in current_core_holdings]
        core_slots = min(len(core_new_candidates), max_new, STOCK_MAX_HOLDINGS - n_core_retained)
        ...
        # Step 5: Defense fill (only if industry didn't fill all slots)
        if max_total_pos >= 1.0:
            defense_fill_max = DEFENSE_FILL_MAX_BULL
        else:
            defense_fill_max = DEFENSE_FILL_MAX_BEAR
        
        # Defense slots after core buy
        max_new_after_core = max_new - core_slots
        defense_new_candidates = [t for t in defense_candidates if t not in current_defense_holdings]
        defense_slots = min(len(defense_new_candidates), max_new_after_core, DEFENSE_MAX_HOLDINGS - n_defense_retained)
        
        # Defense fill target: fills the gap between industry and max_total_pos, up to defense_fill_max
        if industry_target < max_total_pos and defense_slots > 0:
            gap_to_fill = max_total_pos - industry_target
            # Defense fill amount = min(gap, defense_fill_max, available slots * per-ETF target)
            # Each defense ETF: no explicit single max, but typically ~15%
            defense_fill_target = min(gap_to_fill, defense_fill_max)
            # But also limited by defense slots * 15% (practical)
...
csv_path = 'reports/b0_target_actual_audit_v3.csv'
...
if max_diff < 0.02:
    ...
    # Root cause analysis for over-target days
    if len(over_target) > 0:
        print(f"\n  Root cause analysis for over-target days:")
        print(f"    Avg core candidates: {over_target['core_candidates'].mean():.1f}")
        print(f"    Avg core retained: {over_target['core_retained'].mean():.1f}")
        print(f"    Avg core slots: {over_target['core_slots'].mean():.1f}")
        print(f"    Avg industry target: {over_target['industry_target'].mean():.1%}")
        print(f"    Avg actual industry: {over_target['actual_industry_ratio'].mean():.1%}")
    
    if len(under_target) > 0:
        print(f"\n  Root cause analysis for under-target days:")
        print(f"    Avg core candidates: {under_target['core_candidates'].mean():.1f}")
        print(f"    Avg core retained: {under_target['core_retained'].mean():.1f}")
        print(f"    Avg core slots: {under_target['core_slots'].mean():.1f}")
        print(f"    Avg industry target: {under_target['industry_target'].mean():.1%}")
        print(f"    Avg actual industry: {under_target['actual_industry_ratio'].mean():.1%}")

print("\n" + "=" * 80)
print("B0 Target vs Actual Position Audit v3 Complete")
print("=" * 80)
